...
class Alumno {
    ...
    public function getAll() {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM alumnoadministracion");
            $stmt->execute();
    ...
    public function getOne($idalumno) {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM alumnoadministracion WHERE idalumno = ?");
    ...
    public function exists($idalumno) {
        try {
            $stmt = $this->pdo->prepare("SELECT COUNT(*) FROM alumnoadministracion WHERE idalumno = :idalumno");
    ...
    public function create($alumno) {
        ...
        try {
            $stmt = $this->pdo->prepare("INSERT INTO alumnoadministracion (idalumno, alumno, apaterno, amaterno, direccion, telefono) VALUES (:idalumno, :alumno, :apaterno, :amaterno, :direccion, :telefono)");
    ...
    public function update($alumno) {
        try {
            $stmt = $this->pdo->prepare("UPDATE alumnoadministracion SET alumno = :alumno, apaterno = :apaterno, amaterno = :amaterno, direccion = :direccion, telefono = :telefono WHERE idalumno = :idalumno");
            return $stmt->execute($alumno);
        } catch (Exception $e) {
    ...
    public function delete($idalumno) {
        try {
            $stmt = $this->pdo->prepare("DELETE FROM alumnoadministracion WHERE idalumno = :idalumno");
